...
#ifndef CUBE_SERVICE_IOCP_CONNECTOR_H_
#define CUBE_SERVICE_IOCP_CONNECTOR_H_
#include <list>
#include <vector>
#include <process.h>
...
class client { //an unblocked iocp connector
public:
	client();
	virtual ~client();
...
public:
	...
	void set_max_conns(int maxconns) {
		_max_conns = maxconns;
	}
	int get_max_conns() {
		return _max_conns;
	...
	void set_max_events(int maxevt) {
		_max_events = maxevt;
	}
	int get_max_events() {
		return _max_events;
	...
	void set_dispatch(dispatch_t type) {
		_dispatch = type;
	}
	dispatch_t get_dispatch() {
		return _dispatch;
	}
...
client<handler>::client() :
		_max_conns(100000), _max_events(256), _max_waittm(10), _dispatch(round), _hd_counter(0), _worker_num(0), _hdl(
				INVALID_HANDLE_VALUE), _thread_id(0), _connector_stop(true) {
...
template<class handler>
int client<handler>::start(int worker_num) {
	/*initial member*/
	...
	_hdl = (HANDLE) _beginthreadex(0, 0, connect_thread, this, 0, &_thread_id);
	if (_hdl == 0) {
...
template<class handler>
int client<handler>::connect(unsigned long ip, unsigned short port, void *arg) {
	/*create socket*/
	...
	/*connect to remote*/
	err = WSAConnect(sock, (struct sockaddr*) &addr, sizeof(addr), 0, 0, 0, 0);
	if (err != 0) {
...
int client<handler>::stop() {
	...
	/*stop iocp workers*/
	vector<worker*>::iterator iter = _workers.begin(), iter_end = _workers.end();
...
template<class handler>
unsigned int client<handler>::concurrency() {
	int conn = 0;
	for (int i = 0; i < _worker_num; i++)
...
template<class handler>
void client<handler>::push(handler *hd) {
	EnterCriticalSection(&_pcrc);
...
template<class handler>
void client<handler>::fetch(list<handler*> &outlst) {
	EnterCriticalSection(&_pcrc);
...
template<class handler>
int client<handler>::dispatch(handler *hd) {
	if (_dispatch == round) {
...
template<class handler>
unsigned client<handler>::connect_thread(void *arg) {
	/*pending handlers wait for connect result*/
	list<handler*> hds;

	client *connector = (client *) arg;
	while (!connector->_connector_stop) {
		...
		if (!hds.empty()) {
			...
			while (iter != iter_end) {
				...
				int fds = select(0, 0, &writefds, &exptfds, &tv);
				if (fds > 0) {
					...
					hds.erase(iter++);
				} else { //no operation ready on the socket, check the next one
					iter++;
				}
			}
		} else {/*sleep for 10ms*/
...
#endif /* CUBE_SERVICE_IOCP_CONNECTOR_H_ */
