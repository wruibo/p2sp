...
client<handler>::client() :
		_epoll_fd(-1), _plock(0), _thread_connector(0), _connector_stop(true), _worker_num(0),
		_hd_counter(0),	_max_conns(100000), _max_events(256), _max_waittm(10), _dispatch(round) {
...
template<class handler>
int client<handler>::push(handler *hd) {
	int ret = 0;
	pthread_rwlock_wrlock(&_plock);
...
template<class handler>
int client<handler>::remove(handler *hd) {
	pthread_rwlock_wrlock(&_plock);
...
template<class handler>
int client<handler>::set_nonblock(int fd) {
...
template<class handler>
void* client<handler>::connect_thread(void *arg) {
