...
#ifndef CUBE_SERVICE_IOCP_IOCP_H_
#define CUBE_SERVICE_IOCP_IOCP_H_
#include <vector>
#include <process.h>
...
public:
	...
	int start(unsigned short port, int worker_num, void *arg = 0);

	/*start accepter with local @ip & listen @port and @worker number, pass @arg to handler's recall method*/
	int start(unsigned int ip, unsigned short port, int worker_num, void *arg = 0);
...
server<handler>::server() :
		_local_ip(0), _listen_port(0), _listen_sock(INVALID_SOCKET), _hdl(INVALID_HANDLE_VALUE), _thread_id(0), _accepter_stop(true),
		_worker_num(0),	_hd_counter(0), _arg(0), _max_conns(100000), _max_events(256), _max_waittm(10), _dispatch(round) {
...
template<class handler>
int server<handler>::start(u_short port, int worker_num, void *arg/* = NULL*/) {
	return start(INADDR_ANY, port, worker_num, arg);
}

template<class handler>
int server<handler>::start(u_int ip, u_short port, int worker_num, void *arg/* = NULL*/) {
	/*initial member*/
	_local_ip = ip;
	_listen_port = port;
	...
	_hdl = (HANDLE)
	_beginthreadex(0, 0, accept_thread, this, 0, &_thread_id);
	if (_hdl == 0) {
		_accepter_stop = true;
...
template<class handler>
unsigned int server<handler>::concurrency() {
	int conn = 0;
	for (int i = 0; i < _worker_num; i++)
...
template<class handler>
int server<handler>::dispatch(handler *hd) {
	if (_dispatch == round) {
...
template<class handler>
unsigned server<handler>::accept_thread(void *arg) {
	server<handler> *accepter = (server<handler> *) arg;
...
#endif /* CUBE_SERVICE_IOCP_IOCP_H_ */
