#ifndef CUBE_SERVICE_EPOLL_ACCEPTER_H_
#define CUBE_SERVICE_EPOLL_ACCEPTER_H_
...
server<handler>::server() :
		_local_ip(0), _listen_port(0), _listen_sock(-1), _accepter_stop(true), _worker_num(0), _hd_counter(0),
		_arg(0), _max_conns(100000), _max_events(256), _max_waittm(10), _dispatch(round), _thread_accepter(0) {
...
template<class handler>
int server<handler>::start(unsigned short port, int worker_num, void *arg /* = NULL */) {
	return start(INADDR_ANY, port, worker_num, arg);
}

template<class handler>
int server<handler>::start(unsigned int ip, unsigned short port, int worker_num, void *arg/*=NULL*/) {
	if (worker_num <= 0)
		return -1;
...
template<class handler>
int server<handler>::set_nonblock(int fd) {
...
template<class handler>
void* server<handler>::accept_thread(void *arg) {
	...
	while (!accepter->_accepter_stop) {
		...
		if (sock >= 0) { //new connection
			if (set_nonblock(sock) != 0) { //set nonblock for the socket, if failed, close it and continue
				close(sock);
				continue;
			}
