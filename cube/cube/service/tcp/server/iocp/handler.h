...
/*iocp operation*/
typedef enum {
	IOCP_SEND, IOCP_RECV, IOCP_INVALID
} iocp_opt;
...
struct iocp_overlapped {
	...
	iocp_overlapped(const char *data, int send_sz, SOCKET sock) :
			_opt(IOCP_SEND), _sock(sock) {
		_buf.len = send_sz;
	...
	iocp_overlapped(int recv_sz, SOCKET sock) :
			_opt(IOCP_RECV), _sock(sock) {
		_buf.len = recv_sz;
